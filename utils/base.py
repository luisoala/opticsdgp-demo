...
def get_mlflow_model_by_name(experiment_name, run_name, 
    ...
    runs = mlflow.search_runs(experiment.experiment_id) #returns a pandas data frame where each row is a run (if several exist under that name)
    client = MlflowClient() 
...
def fetch_from_mlflow(uri, use_cache=True):
